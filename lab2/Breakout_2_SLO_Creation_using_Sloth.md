...
1. **Defining SLOs as YAML Files:** We will use YAML files to define our Service Level Objectives (SLOs).

2. **Generating Recording Rules for Mimir:** Using [Sloth](https://sloth.dev/) we will generate recording rules and alerts based off those SLO YAML files.

3. **Importing Recording Rules into Mimir:** We will use [Mimirtool](https://grafana.com/docs/mimir/latest/manage/tools/mimirtool/) to import the generated recording rules into Mimir.

4. **Tracking SLOs with Grafana Dashboards:** Finally, we will monitor and track our SLOs using Grafana dashboards to ensure our services are meeting the defined objectives.
...
We have been tasked with establishing an Service Level Objectives (SLOs) for our `/login` endpoint.

We will be using the `traces_spanmetrics_calls_total` prometheus metric as our Service Level Indicator metric. This metric is generated by the [tempo metric generator](https://grafana.com/docs/tempo/latest/).
